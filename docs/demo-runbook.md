...
6. The post-call package can be sent over the configured WhatsApp transport when the caller requested it.

## Callback states

`pending` -> `dispatching` -> `dispatched` -> `completed`

- `disabled`: due work remains `pending` because outbound calling is off.
- `dry_run`: records a simulated dispatch without placing a call.
- `failed`: a conclusive provider rejection can be retried under the bounded retry policy.
...
| Post-call material | Contextual WhatsApp message and approved artifacts |
